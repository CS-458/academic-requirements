...
export const Container: FC<ContainerProps> = memo(function Container({
        ...
        courseStringArr.forEach((courseString: string) => {
          ...
          PassedCourseList.forEach((x) => {
            if (
              ...
              !CompletedCourses.includes(`${x.subject}-${x.number}`)
            ) {
              if (!foundOnce) {
  ...
  function selectedCategory(_category: any): void {
    ...
    PassedCourseList.forEach((course, _index) => {
  ...
  function checkCourseSemester(course: Course, semNum: number): boolean {
    return (
      ...
      semesters.forEach((semester, index) => {
        semester.courses.forEach((course) => {
          ...
          if (
            ...
            setVisibility(true);
            setErrorMessage(
              `WARNING! ${course.subject}_${course.number}is already in other semesters.`
            );
    ...
    if (updateWarning.newCheck) {
      ...
      if (
        ...
        if (
          ...
          setVisibility(true);
          setErrorMessage(
            `WARNING! ${updateWarning.course.subject}_${
    ...
    if (updateWarning.newCheck) {
      ...
      if (!updateWarning.draggedOut) {
        ...
        if (!satisfied.returnValue) {
          setVisibility(true);
          setErrorMessage(
            `WARNING! ${updateWarning.course.subject}_${updateWarning.course.number} has failed the following prerequisites: ${satisfied.failedString}`
          );
  ...
  function preReqCheckAllCoursesPastSemester(
    ...
    if (showMessage && failedCoursesList.length > 0) {
      ...
      if (
        ...
        setVisibility(true);
        setErrorMessage(
          `WARNING! ${courseToRemove.subject}_${courseToRemove.number} is a prerequisite for the following courses: ${message}`
        );
      }
    }
  ...
  function getPreviousSemesterCourses(semesterIndex: number): string[] {
  ...
  // param semesterIndex -> current semester index
  function getSemesterCourses(semesterIndex: number): Course[] {
  ...
  // param semesterIndex -> current semester index
  function getSemesterCoursesNames(semesterIndex: number): string[] {
  ...
  const getWarning = (SemesterCredits: any): string => {
    let Warning = "";
  ...
  useEffect(() => {
        ...
        for (let i = 0; i < reqGenList.length; i++) {
          if (reqGenList[i].idCategory === x.parentCategory) {
            ...
            if (reqGenList[i].courseReqs == null) {
              ...
              reqGenList[
                i
              ].courseReqs = `${reqGenList[i].courseReqs},${x.courseReqs}`;
            }
            reqGenList[i].inheritedCredits = x.creditCount;
    ...
    for (let i = 0; i < PassedCourseList.length; i++) {
      ...
      if (!skip) {
        const currentIdString = `${PassedCourseList[i].subject}-${PassedCourseList[i].number}`;
        ...
        if (index > -1 && index !== undefined) {
          ...
          if (reqGenList[i].courseReqs != null) {
            ...
            courseReqArr.forEach((item) => {
              const found = reqCheck.courseInListCheck(item, [courseString]);
  ...
  const checkRequirements = useCallback(
    (course: Course, multipleCategories: any) => {
      ...
      const Major = checkRequirementsMajor(course);
      if (!Major) {
  ...
  function checkRequirementsGen(course: Course, multipleCategories: any) {
    ...
    const reqCheck = new StringProcessing();
    // run once or for each category the course is in
    for (let n = 0; n < (categories ? categories.length : 1); n++) {
      const courseCategory = categories ? categories[n] : course.idCategory;
          ...
          if (x.coursesTaken.indexOf(courseString) == -1) {
            ...
            if (x.parentCategory) {
              ...
              if (parent.courseReqs != null) {
                ...
                let validCourse = false;
                courseReqArr.forEach((item) => {
                  const found = reqCheck.courseInListCheck(item, [
                    courseString
                  ]);
                  if (found.returnValue) {
                    validCourse = true;
